...
#pragma endregion
	...
	void OPPRFSender::sendSSPolyBased(u64 IdxP, binSet& bins, std::vector<block>& plaintexts, const std::vector<Channel*>& chls)
	{
		...
		gTimer.setTimePoint("online.send.spaw");
		Log::out << "bin.size[" << bins.mSimpleBins.mBinCount[0] << "]: " << Log::endl;
		Log::out << "bin.size[" << bins.mSimpleBins.mBinCount[1] << "]: " << Log::endl;
				...
				for (auto bIdxType = 0; bIdxType < 2; bIdxType++)
				{
					...
					for (u64 bIdx = binStart; bIdx < binEnd;)
					{
						...
						for (u64 stepIdx = 0; stepIdx < currentStepSize; ++bIdx, ++stepIdx)
						{
							...
							Log::out << "bin.mIdx[" << bIdx << "]: start" <<  Log::endl;
							...
							if (bin.mIdx.size() > 0)
							{
								...
								if (bIdx == binCountSend) //TODO: weird error for bIdx== binCountSend, hack for perfomance
								...
								for (u64 i = 0; i < bins.mSimpleBins.mMaxBinSize[bIdxType]; ++i)
								{
									if (bIdx != binCountSend) //TODO: weird error for bIdx== binCountSend, skip for now
										memcpy(
							...
							Log::out << "bin.mIdx[" << bIdx << "]: done " << Log::endl;
