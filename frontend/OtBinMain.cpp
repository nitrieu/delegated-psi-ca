...
//static std::string filePath = "../../testout.txt";
...
{
	opt = 1; //0: Table - based; 1: POLY - seperated; 2 - POLY - combined; 3 - BloomFilter
	nParties = 2;
	u64  psiSecParam = 40, bitSize = 128, numThreads = 1;
	PRNG prng(_mm_set_epi32(4253465, 3434565, 234435, 23987045));
	...
	if (myIdx == 0)
	{
		for (int i = 0; i < 5; i++)
		{
			Log::out << sendPayLoads[i] << "\t send \t " << Log::endl;
			//Log::out << recvPayLoads[2][i] << Log::endl;
		}
		Log::out << "------------" << Log::endl;
	}
	...
	//	sendMaskBuff->resize(sendPayLoads.size()* bins.mMaskSize*sizeof(u8));
	//	auto maskView = sendMaskBuff->getMatrixView<u8>(bins.mMaskSize * sizeof(u8));

	//	for (u64 i = 0; i < sendPayLoads.size(); ++i)
	//		memcpy(maskView[i].data(), (u8*)&sendPayLoads[i], bins.mMaskSize);
	//}
	//else if (myIdx == 1) {
	//	//I am a recv to my previous neigbour
	//	recv.getOPRFkeys(0, bins, chls[0], false);
	...
	if (myIdx == 0)
	{
		...
		for (u64 i = 0; i < setSize; ++i)
		{
			...
			//auto sendMask = ZeroBlock;
			//memcpy(&sendMask, (u8*)&sendPayLoads[i], bins.mMaskSize);

			//auto recvMask = ZeroBlock;
			//memcpy(&recvMask, (u8*)&recvPayLoads[i], bins.mMaskSize);

			//std::cout << IoStream::lock;
			//for (int i = 0; i < 5; i++)
			//{
			//	Log::out << "sendMask: " << sendMask << Log::endl;
			//	Log::out << "recvMask: " << recvMask <<  Log::endl;
			//}
			//std::cout << IoStream::unlock;
